...
IMPROVE_MODEL = "anthropic/claude-haiku-4-5"


def _run_opencode(prompt: str, model: str) -> str:
    """Run opencode run with a prompt and return the assistant text response."""
    env_clean = {
        k: v
        for k, v in __import__("os").environ.items()
        if k not in ("OPENCODE", "OPENCODE_PID")
    }
    ...
    # Parse JSON event stream, collect text parts
    text_parts = []
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    text = _run_opencode(prompt, model)
    ...
    if len(description) > 1024:
        ...
        shorten_text = _run_opencode(shorten_prompt, model)
        match = re.search(
...
def main():
    ...
    parser.add_argument("--model", default=IMPROVE_MODEL, help="Model for improvement")
    parser.add_argument(
